...
<body class="bg-[#FFFBF5] min-h-screen p-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-10 px-4">
        ...
        <div class="flex items-center gap-3 mb-4 md:mb-0">
            <img src="{{ asset('images/logo.png') }}" alt="Logo" class="w-10 h-10 rounded-full object-cover shadow-md border border-[#F9A826]">
            <h1 class="text-2xl font-bold text-black">Kafe Angin Semilir</h1>
        </div>

        <!-- Menu Navigasi (Bentuk Pill) -->
        <div class="flex gap-3">
            <a href="{{ url('/') }}" class="px-6 py-2 rounded-full bg-white text-gray-600 font-semibold shadow-sm hover:bg-gray-100 transition text-sm flex items-center">
                Beranda
            </a>
    ...
    <div class="mb-6 px-2">
        <a href="{{ route('admin.menu.create') }}" class="inline-flex items-center gap-2 bg-[#F9A826] text-white px-6 py-3 rounded-full font-bold shadow-lg hover:bg-[#e08e10] transition transform hover:-translate-y-1">
            ...
            <table class="w-full border-collapse">
                ...
                <tbody class="text-gray-700 text-sm">
                    @forelse ($menus as $index => $menu)
                        ...
                        <tr class="{{ $loop->even ? 'bg-[#FFF6E5]' : 'bg-white' }} border-b border-gray-100 hover:bg-orange-50 transition">

                            <!-- Kolom ID -->
                            <td class="py-4 px-4 text-center font-bold">{{ $index + 1 }}</td>

                            <!-- Kolom Gambar Bulat -->
                            <td class="py-4 px-4 text-center">
                                <div class="w-16 h-16 rounded-full overflow-hidden border-2 border-white shadow-md mx-auto">
                                    @if($menu->foto)
                                        <img src="{{ asset('foto_menu/' . $menu->foto) }}" class="w-full h-full object-cover">
                                    @else
                            ...
                            <!-- Kolom Nama Menu -->
                            <td class="py-4 px-4 font-semibold text-gray-800">{{ $menu->nama }}</td>
                            ...
                            <td class="py-4 px-4 text-center">
                                <div class="flex justify-center gap-2">
                                    <!-- Tombol Edit -->
                                    <a href="{{ route('admin.menu.edit', $menu->id) }}" class="w-9 h-9 rounded bg-[#FFD700] text-black flex items-center justify-center shadow-sm hover:bg-yellow-400 transition">
